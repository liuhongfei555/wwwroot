<template>
	<view class="fa-poster">
		<view class="" v-if="showCanvas">
			<canvas canvas-id="poster" class="poster_canvas" :style="[{ width: width + 'px', height: height + 'px' }]"></canvas>
		</view>
	...
	methods: {
		...
		getWxCode() {
			return new Promise((resolve, reject) => {
				this.$api.getWxCode({ project_id: this.info.id }).then(({ code, data: res, msg }) => {
					if (code) {
						resolve(res);
					} else {
						reject(msg);
		...
		async initCanvas() {
			//先取下载图片
			if(!this.info.images_text.length){
				this.$u.toast('请上传项目图片');
				this.hide();
				return;
			}
			const infoImgPath = await this.downloadImg(this.info.images_text[0]);
			//取图片信息
			const { width, height } = await this.getImageInfo(infoImgPath);
			...
			this.$nextTick(() => {
				this.createPoster(infoImgPath);
			});
		},
		// 创建海报
		async createPoster(infoImgPath) {
			try{
				if (this.posterImgs) {
					return;
				}
				...
				ctx.fillRect(0, 0, this.width / 2, this.height / 2);
				this.drawImage(ctx, infoImgPath);
				const codeUrl = await this.getWxCode();
				const left_gap = 17*this.multiple;
				//标题			
				let drawtextList = this.drawtext(this.info.title, this.width / 2 - 30*this.multiple);			
				...
				ctx.fillText(this.info.price==0?'免费':this.info.price, left_gap, textTop + 47*this.multiple);				
				//分割线
				ctx.beginPath();
				ctx.setLineWidth(1*this.multiple);
				ctx.moveTo(17, textTop + 70*this.multiple);
				...
				ctx.fillText('预约', left_gap, textTop + 150*this.multiple);				
				// // 二维码
				// #ifdef MP-WEIXIN
